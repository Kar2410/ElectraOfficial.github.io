...
{
    global $conn;
    $sql = "SELECT p.*, u.username FROM announcements AS p JOIN users AS u ON p.user_id=u.id WHERE p.published=? ORDER BY p.created_at DESC";
...
function get_post_with_username_announce()
{
    global $conn;
    $sql = "SELECT p.*, u.username FROM announcements AS p JOIN users AS u ON p.user_id=u.id  ORDER BY p.created_at DESC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $records = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    return $records;
